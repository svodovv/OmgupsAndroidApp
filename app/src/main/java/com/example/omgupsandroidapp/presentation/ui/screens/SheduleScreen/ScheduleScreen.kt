...
fun ScheduleScreen(
    ...
    val nechetWeek = nechet.filter { it.type_of_week == 0 }
    ...
    Log.i("NECHET",nechpon1.toString())
    LazyColumn(
        modifier = Modifier
            .padding(0.dp, 60.dp)
            .fillMaxSize(),
        ...
        item {
            Column(
                ...
                for (it in dayOfWeek) {
                    when (it) {
                        "Понедельник" -> Row {
                                createDayBox(dayOfWeek = it, schedule = nechetWeek)
                        }
                        "Вторник" -> Row {
...
fun createDayBox(dayOfWeek: String, schedule: List<SheduleModel>) {
        ...
        Column(
            ...
            for (scheduleItem in schedule) {
                schedule.map {
                    when (it.day_of_week) {
                        dayOfWeek -> when (scheduleItem.time) {
                            0 -> Row {
                                Column(
                                    modifier = Modifier
                                        .padding(10.dp, 10.dp)
